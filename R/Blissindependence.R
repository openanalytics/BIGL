...
Blissindependence <- function(doseInput, parmInput, ...){
  ...
  pred1 <- pred1*abs(pars["m1"]-pars["b"])/max(abs(pars["m1"]-pars["b"]), abs(pars["m2"]-pars["b"]))
  pred2 <- Hilleq(doseInput[, "d2"], pars["h2"], pars["e2"])
  pred2 <- pred2*abs(pars["m2"]-pars["b"])/max(abs(pars["m1"]-pars["b"]), abs(pars["m2"]-pars["b"]))
  ...
  applyfunction <- function(param) {
    ...
    predcombo <- param[3] + rise*max(abs(param[4]-param[3]), abs(param[5]-param[3]))*predcombo
    return(predcombo)
  }
