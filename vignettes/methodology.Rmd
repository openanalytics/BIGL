...
equal across all points. This assumption is also mentioned in the orignal
...
off-axis dose-combinations seperatly. Two extra methods are described below: the
`SepVar` method (Separated variance) and the `ModVar` method (Modelled
...
and 
(b) the dose combinations (= $\sigma^{2}_{1}$).
The notation for both `SepVar` as `ModVar` will be the same, but the estimation
of $\sigma^{2}_{1}$ will be different. The variance of the monotherapies is
still estimated the same as before by taking the MSE of the null model. The test
statistic is given by:

$$
\operatorname{TestStat} = \frac{R^{T}\left(\sigma^{2}_{1}D + \sigma^{2}_{0}C_{p}\right)^{-1}R}{n_{1}}
$$
...
1. **`SepVar` method**: The variance for the dose combinations is estimated by
   ...
   these variances. The downside of this method is that the variance for all
   combinations is assumed to be equal. In reality the variance often depends
   on the mean effect. This is taken into account in the `ModVar` method.

2. **`ModVar` method**: In this method $\sigma^{2}_{1}$ is no longer estimated
   as a single number but rather as a vector of variances. Each off-axis point
   has now its own variance. A linear model is fitted on the original dataset,
   modelling the variance of each off-axis point as a function of its
   mean effect. The estimated model parameters are then used to predict the
   variance for the corresponding mean effect measured for that dose
   combination. These predicted variances are placed in the
   vector $\sigma^{2}_{1}$.
...
estimates are used. The estimates for $\sigma^{2}_{1}$ are different depending
...
$$
\max \left| R^{T}\left(\sigma^{2}_{1} D + \sigma^{2}_{0} C_{p}\right)^{-1/2} \right|  \sim \max \left| Z_{1}, \dots, Z_{k} \right|
$$
...
$$
\operatorname{TestStat}\left(d_{1}, d_{2}\right) = \left[\left| R^{T}\left(\sigma^{2}_{1} D + \sigma^{2}_{0} C_{p}\right)^{-1/2} \right| \right]_{k}
$$
...
higher with the methods assuming variance heterogenity compared to the methods
