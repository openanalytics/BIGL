...
In addition, a contour plot can be made with pointwise confidence intervals, similar to the `drugCombo` package.
...
For the variance model, an exploratory plotting function is available.

```{r modelVariancePlot, fig.width=6, fig.height=5}
plotMeanVarFit(data)
plotMeanVarFit(data, trans ="log")
```
