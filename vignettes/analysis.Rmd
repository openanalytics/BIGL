...
The panels correspond to concentration levels of one compound and the x-axis shows the concentration levels of the second compound.
The user has the option to define which compound will be shown in the panels and in the x-axis.

```{r plot_2d_cross_section, message=FALSE, comment = NA, fig.width = 8, fig.height = 6}
nullModels <- c("loewe", "loewe2", "bliss", "hsa")
rs_list <- Map(fitSurface, null_model = nullModels, MoreArgs = list(data = data, fitResult = marginalFit, B.CP = 50, statistic = "none", parallel = FALSE))

synergy_plot_bycomp(rs_list, ylab = "Response", color = TRUE)
```
...
Contour plot colouring can be defined according to the effect sizes or according to maxR results.

```{r plotSingleConfInt, fig.height=5, fig.width=8}
...
```{r contour_effectsize, warning=FALSE, fig.align="center", fig.width=6, fig.height=5, message=FALSE, comment = NA}
contour(
    maxR_B,
...
```{r plot3d_effectsize, warning=FALSE, fig.height=7, fig.width=7, message=FALSE, comment = NA}
